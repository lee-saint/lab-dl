...
if __name__ == '__main__':
    ...
    # (n, m) array와 (n, ) 1차원 array의 경우 1차원 array를 (n, 1) shape로 reshape 해야 함
    ...
    print(X)
    # 1. X의 모든 원소 중 최댓값(m)을 찾아 X - m을 계산해서 출력
    m = np.max(X)
    print('m =', m)
    result = X - m  # (2, 3) shape의 2차원 배열과 스칼라 간의 broadcast
    print(result)

    # 2. X의 axis=0 방향 최댓값(각 컬럼에서의 최댓값)을 찾아 X의 각 원소에서 그 컬럼의 최댓값을 뺀 행렬 출력
    m = np.max(X, axis=0)
    print(f'm = {m}, shape: {m.shape}')
    result = X - m
    print(result)

    # 3. X의 axis=1 방향 최댓값(각 row에서의 최댓값)을 찾아 X의 각 원소에서 그 row의 최댓값을 뺀 행렬 출력
    m = np.max(X, axis=1)
    print(f'm = {m}, shape: {m.shape}')
    ...
    print(X_t)
    m = np.max(X_t, axis=0)  # 전치 행렬에서 axis=0 방향으로 최댓값 찾기
    result = X_t - m  # 전치 행렬에서 최댓값 빼기
    result = result.T  # 전치 행렬을 다시 한번 transpose
    print(result)

    # 4. X의 각 원소에서 그 컬럼의 최댓값을 뺀 행렬의 컬럼별 합계
    m = np.max(X, axis=0)
    result = X - m
    s = np.sum(result, axis=0)
    print(s)

    # 5. X의 각 원소에서 그 row의 최댓값을 뺀 행렬의 row별 합계
    m = np.max(X, axis=1)
    result = X - m.reshape((2, 1))
