...
class TwoLayerNetwork:
    def __init__(self, input_size, hidden_size, output_size, weight_init_std=0.01):
        """입력: input_size (ex. 784(28x28)개)
        첫번째 층(layer)의 뉴런 개수: hidden size (ex. 32개)
        출력 층(layer)의 뉴런 개수: output (ex. 10개)
    ...
    def softmax(self, x):
        ...
        if x.ndim == 1:
            max_x = np.max(x)
            x -= max_x  # 오버플로우 방지를 위해
            result = np.exp(x) / np.sum(np.exp(x))
        else:  # ndim==2
            x_t = x.T  # 행렬 x의 전치행렬(transpose) 만들기
    ...
    def accuracy(self, x, y_true):
        """
        x의 예측값과 실제값 y_true를 사용해 정확도(맞춘 개수/전체 개수)를 계산
        실제값(y_true)은 one-hot-encoding 되어있다고 가정
        :param x: 예측값을 구하고 싶은 데이터. 2차원 ㅂ ㅐ열
...
if __name__ == '__main__':
    ...
    print(f"W2: {neural_net.params['W2'].shape}, b2: {neural_net.params['b2'].shape}")

    # 신경망 클래스의 predict() 메소드 테스트
    # mnist 데이터세트를 로드(dataset.load_mnist 사용)
    ...
    # 찾은 gradient를 이용해서 weight/bias 행렬을 업데이트
